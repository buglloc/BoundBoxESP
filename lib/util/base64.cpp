...
namespace
{
  ...
  size_t decodeBase64Length(const String& input) {
    size_t b64chars = 0;
    for (auto&& ch: input) {
      if (base64ToBinary(input[0]) >= 64) {
        break;
      }
      b64chars++;
    }
...
namespace BBU
{
  ...
  {
    size_t fullSets = bytes.length()/3;
    String out("\xff", encodeBase64Length(bytes.length()));
  ...
  Bytes Base64Decode(const String& in)
  {
