...
namespace Peripheral
{
  ...
  {
  #if CONFIG_BBP_NET_USE_STATIC_IP
  ...
  {
  #ifndef CONFIG_BBP_NET_DNS
    return;
  #else

    esp_netif_dns_info_t dns;
    dns.ip.u_addr.ip4.addr = ipaddr_addr(CONFIG_BBP_NET_DNS);
  ...
  inline void SetIPInfo(esp_netif_t *netif)
  {
    ...
    ip.ip.addr = ipaddr_addr(CONFIG_BBP_NET_IP);
    ip.netmask.addr = ipaddr_addr(CONFIG_BBP_NET_SUBNET);
    ip.gw.addr = ipaddr_addr(CONFIG_BBP_NET_GW);
